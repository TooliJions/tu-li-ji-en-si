...
function createProvider(): DashScopeProvider {
  return new DashScopeProvider({ apiKey: 'sk-test', model: 'qwen3.6-plus' });
}
...
describe('DashScopeProvider', () => {
  ...
  describe('generate', () => {
    it('返回文本和用量', async () => {
      mockCreate.mockResolvedValue({
        choices: [{ message: { content: 'Hello from DashScope' } }],
      ...
      expect(result.text).toBe('Hello from DashScope');
  ...
  describe('默认配置', () => {
    it('未提供 baseURL 时使用默认端点', () => {
      const p = new DashScopeProvider({ apiKey: 'test', model: 'qwen' });
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect((p as any).config.baseURL).toBe('https://dashscope.aliyuncs.com/compatible-mode/v1');
    });
  });
});
