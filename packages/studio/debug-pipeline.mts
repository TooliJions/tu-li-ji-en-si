...
import * as pipelineModule from './src/api/routes/pipeline.ts';
import * as booksModule from './src/api/routes/books.ts';
import * as coreBridgeModule from './src/api/core-bridge.ts';

const app = new Hono();
app.route('/api/books', booksModule.createBookRouter());
app.route('/api/books/:bookId/pipeline', pipelineModule.createPipelineRouter());

pipelineModule.pipelineStore.clear();
booksModule.resetBookStoreForTests();
coreBridgeModule.resetStudioCoreBridgeForTests();
