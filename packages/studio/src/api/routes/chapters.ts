...
function isLegacyAuditReport(report: unknown): boolean {
...
function readNormalizedAuditReport(
  bookId: string,
  chapterNumber: number,
  content: string
): unknown | null {
...
export function readChapterRecord(bookId: string, chapterNumber: number): ChapterRecord | null {
  ...
  const auditReport = readNormalizedAuditReport(bookId, chapterNumber, parsed.content);
